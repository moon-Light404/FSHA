...
class FSHA:
    ...
    def train_step(self, x_private, x_public, label_private, label_public):
        ...
        if 'gradient_penalty' in self.hparams:
            ...
            D_loss += D_gradient_penalty * w    # 对鉴别器D进行梯度惩罚正则化
        ...
        return f_losses.cpu(), tilde_f_losses.cpu(), D_losses.cpu(), losses_c_verification.cpu()

    # 梯度惩罚是对鉴别器D进行正则化，只更新鉴别器D的参数，不对其他网络f,tilde_f进行更新
    def gradient_penalty(self, x, x_gen):
        epsilon = torch.rand([x.shape[0], 1, 1, 1]).cuda()
        x_hat = epsilon * x + (1 - epsilon) * x_gen
        x_hat = torch.autograd.Variable(x_hat, requires_grad=True) # 对x_hat进行求导
